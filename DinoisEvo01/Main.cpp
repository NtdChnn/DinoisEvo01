...
int main()
{
	...
	RectangleShape playerHitBox(Vector2f(100.0f, 70.0f));
	//BG
	...
	playerHitBox.setOrigin(300.0f, -105.0f);
	...
	player.setTexture(&playerTexture01);
	...
	Animation animationplayer(&playerTexture01, Vector2u(8, 1), 0.05f);
	...
	while (window.isOpen())
	{
		...
		if (yplayer == 350 && pause == 0 && gameOver == 0)
		{
			jump = 0;
		}
		...
		if (rock01.getGlobalBounds().intersects(playerHitBox.getGlobalBounds()))
		{
			move = 0; gameOver = 1;
		}
		if (rock02.getGlobalBounds().intersects(playerHitBox.getGlobalBounds()))
		{
			move = 0; gameOver = 1;
		}
